...
const niproxy = async () => {
  try {
    const nikki_secret = await execute('uci get nikki.mixin.api_secret');
    const nikki_port = await execute('uci get nikki.mixin.api_port')
    const nikki_name = await execute('uci get nikki.mixin.ui_name')
    const nikki_dash = await execute('uci get nikki.mixin.ui_url')
    
    const headers = new Headers({
        "Accept": "application/json, text/plain, */*",
        "Accept-Encoding": "gzip, deflate",
        "Accept-Language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7",
        "Authorization": `Bearer ${nikki_secret}`,
...
const nizeus = async () => {
    ...
    const nikki_dash = await execute('uci get nikki.mixin.ui_url')
    const name = "GLOBAL"
    const url = `http://127.0.0.1:9090/proxies/${name}/delay?timeout=5000&url=https%3A%2F%2Fwww.gstatic.com%2Fgenerate_204`;
    const headers = new Headers({
        "Accept": "application/json, text/plain, */*",
        "Accept-Encoding": "gzip, deflate",
        "Accept-Language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7",
  ...
  if (result.message){
   ...
   return `ping ${name} ${result.delay}ms`
  }
};

export { nizeus, niproxy };