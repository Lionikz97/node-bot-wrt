...
import refreshYacd from './proxysync.js';
...
export const callback = async (bot, query, chatId, messageId, data) => {
	...
	`
			...
			case "reopenwrt":
				bot.editMessageText("loading", options);
				try {
					bot.editMessageText("rebootting ...", {
					    chat_id: chatId,
					    message_id: messageId,
					    "parse_mode": "html"
					});
		...
		if (data.includes('yacd-data')) {
			try {
				...
				if (parseInt(oc.trim(), 10) === 1) {
				    ...
				    app = "nikki";
				    referer = "http://127.0.0.1:9090/ui/Nikki"
				} else if (parseInt(oc.trim(), 10) === 0 && parseInt(neko.trim(), 10) === 0 && parseInt(nikki.trim(), 10) === 0) {
				...
				bot.editMessageText(`all proxies ${parms}, button only supports type selector ⚡\ntype : ${proxies.type}`,{
					chat_id: chatId,
					message_id: messageId,
					parse_mode: "html",
					disable_web_page_preview: true,
			    ...
			    bot.editMessageText(`Error: <code>${e.message || e}</code>`, {
			        chat_id: chatId,
			        message_id: messageId,
			        parse_mode: "html",
			        disable_web_page_preview: true,
    ...
    const groupName = splitt[1].trim();
    const proxyName = splitt[2].trim();
    const url = `http://127.0.0.1:9090/proxies/${groupName}`;
    const requestData = { name: proxyName };
	let secret
	let app
	let config
	let referer
    ...
    try {
        ...
        await refreshYacd(bot, chatId, messageId, groupName, secret, app);

    } catch (e) {
        console.error("Error on switch:", e);
        return bot.sendMessage(chatId, `Failed to switch: ${e.message}`);
    }
		} else
		...
		if (data.includes("passwall") || data.includes("openclash") || data.includes("mihomo") || data.includes("neko")|| data.includes("nikki")) {
			await bot.editMessageText("loading", options);
